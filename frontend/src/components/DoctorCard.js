...
const DoctorCard = ({ doctor }) => {
  const colorDoc = (doctor) => {
    if (!doctor?.specialization?.specialization) return "gray-200"; // safe default
    switch (doctor.specialization.specialization) {
      case "Physician":
        return "blue-300";
      case "Otolaryngologist":
      ...
      <div className='bg-white rounded-lg px-6 py-3 shadow-md'>
        <div className='flex items-start space-x-6'>
          ...
          <div>
            ...
            <p className={`text-sm font-medium text-gray-700 font-fontPro mb-3 bg-${colorDoc(doctor)} inline-block px-1 rounded-md`}>
              {doctor?.specialization?.specialization || "No specialization"}
            </p>
            <p className='text-xs text-gray-700 font-fontPro mb-2'>
              <i className='fas fa-graduation-cap fa-fw'></i>
              <span> {doctor.background}</span>
